#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano diagnostico-operacional-logs` em 2026-08-17 05:48
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo diagnostico-operacional-logs
set -u
set -o pipefail

RUN_ID='study-credit-flow-diagnostico-operacional-logs-mswtc2fh'
FEATURE='diagnostico-operacional-logs'
BASE_BRANCH='spec/diagnostico-operacional-logs'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/diagnostico-operacional-logs/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/study-credit-flow-diagnostico-operacional-logs-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/study-credit-flow-diagnostico-operacional-logs"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/diagnostico-operacional-logs/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/diagnostico-operacional-logs-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-042' 'Você executa UMA tarefa da feature "diagnostico-operacional-logs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/diagnostico-operacional-logs/spec.md, .spec/features/diagnostico-operacional-logs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-042 — "Criar fundação segura de logging estruturado"
  critérios/refs: AC-100 (Console produz JSON estruturado com identidade do serviço), AC-101 (Logs evitam dados sensíveis e volume nominal excessivo)
  arquivos permitidos (e seus testes): src/main/resources/logback-spring.xml, src/main/resources/application-observability.yml, src/test/kotlin/io/github/brdoliveira/creditflow/platform/observability/StructuredLoggingIT.kt
  mensagem de commit: "T-042 diagnostico-operacional-logs: Criar fundação segura de logging estruturado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/diagnostico-operacional-logs-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/diagnostico-operacional-logs-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-043' 'Você executa UMA tarefa da feature "diagnostico-operacional-logs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/diagnostico-operacional-logs/spec.md, .spec/features/diagnostico-operacional-logs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-043 — "Registrar falhas HTTP técnicas"
  critérios/refs: AC-097 (Falhas HTTP técnicas deixam evidência correlacionada), AC-101 (Logs evitam dados sensíveis e volume nominal excessivo)
  arquivos permitidos (e seus testes): src/main/kotlin/io/github/brdoliveira/creditflow/platform/observability/SafeExceptionDetails.kt, src/main/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/web/error/GlobalExceptionHandler.kt, src/test/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/observability/ObservabilityTest.kt, src/test/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/observability/RuntimeObservabilityIT.kt, src/test/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/web/error/GlobalExceptionLoggingTest.kt
  mensagem de commit: "T-043 diagnostico-operacional-logs: Registrar falhas HTTP técnicas"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/diagnostico-operacional-logs-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/diagnostico-operacional-logs-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-044' 'Você executa UMA tarefa da feature "diagnostico-operacional-logs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/diagnostico-operacional-logs/spec.md, .spec/features/diagnostico-operacional-logs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-044 — "Propagar e registrar contexto assíncrono"
  critérios/refs: AC-098 (Retentativa da outbox é observável sem expor o evento), AC-099 (Consumo Kafka mantém correlação e resultado operacional), AC-101 (Logs evitam dados sensíveis e volume nominal excessivo)
  arquivos permitidos (e seus testes): src/main/kotlin/io/github/brdoliveira/creditflow/platform/observability/CorrelationLogContext.kt, src/main/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/outbox/OutboxPublisher.kt, src/main/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/messaging/CreditEvaluationKafkaListener.kt, src/test/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/messaging/AsyncOperationalLoggingTest.kt
  mensagem de commit: "T-044 diagnostico-operacional-logs: Propagar e registrar contexto assíncrono"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/diagnostico-operacional-logs-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/diagnostico-operacional-logs-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-045' 'Você executa UMA tarefa da feature "diagnostico-operacional-logs" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/diagnostico-operacional-logs/spec.md, .spec/features/diagnostico-operacional-logs/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-045 — "Documentar operação e provar compatibilidade"
  critérios/refs: AC-100 (Console produz JSON estruturado com identidade do serviço), AC-101 (Logs evitam dados sensíveis e volume nominal excessivo)
  arquivos permitidos (e seus testes): README.md, docs/architecture.md, .env.example, test/kotlin-architecture.test.mjs, .spec/verification/diagnostico-operacional-logs.json
  mensagem de commit: "T-045 diagnostico-operacional-logs: Documentar operação e provar compatibilidade"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/diagnostico-operacional-logs-faixa-4' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/diagnostico-operacional-logs/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-042"
  echo "  faixa-2  onda 1  T-043"
  echo "  faixa-3  onda 1  T-044"
