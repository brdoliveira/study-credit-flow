#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano fortalecer-arquitetura-modular` em 2026-08-17 03:30
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo fortalecer-arquitetura-modular
set -u
set -o pipefail

RUN_ID='study-credit-flow-fortalecer-arquitetura-modular-mswoei3v'
FEATURE='fortalecer-arquitetura-modular'
BASE_BRANCH='spec/fortalecer-arquitetura-modular'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/fortalecer-arquitetura-modular/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/study-credit-flow-fortalecer-arquitetura-modular-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/study-credit-flow-fortalecer-arquitetura-modular"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/fortalecer-arquitetura-modular/executar-tarefas.sh --faixa $1"
...
# ── faixa-1: T-036 T-037 T-040 ──
executar_faixa_1() {
  local WT="$WT_BASE-faixa-1"
  preparar_worktree 'faixa-1' 'spec/fortalecer-arquitetura-modular-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-036' 'Você executa UMA tarefa da feature "fortalecer-arquitetura-modular" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fortalecer-arquitetura-modular/spec.md, .spec/features/fortalecer-arquitetura-modular/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-036 — "Impor todas as fronteiras arquiteturais"
  critérios/refs: AC-090 (Fronteiras de dependência são verificadas integralmente)
  arquivos permitidos (e seus testes): test/kotlin-architecture.test.mjs, src/test/kotlin/io/github/brdoliveira/creditflow/spec/SpecificationContractTest.kt
  mensagem de commit: "T-036 fortalecer-arquitetura-modular: Impor todas as fronteiras arquiteturais"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high &&
    rodar_tarefa 'faixa-1' 'T-037' 'Você executa UMA tarefa da feature "fortalecer-arquitetura-modular" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fortalecer-arquitetura-modular/spec.md, .spec/features/fortalecer-arquitetura-modular/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-037 — "Consolidar a organização por feature"
  critérios/refs: AC-091 (Testes acompanham os pacotes de produção), AC-092 (Componentes específicos pertencem ao módulo de avaliação)
  arquivos permitidos (e seus testes): src/main/kotlin/io/github/brdoliveira/creditflow/application/event/CreditEvaluationCompleted.kt, src/main/kotlin/io/github/brdoliveira/creditflow/infrastructure/idempotency/CanonicalRequestHasher.kt, src/main/kotlin/io/github/brdoliveira/creditflow/infrastructure/messaging, src/main/kotlin/io/github/brdoliveira/creditflow/infrastructure/observability/CreditMetrics.kt, src/main/kotlin/io/github/brdoliveira/creditflow/infrastructure/outbox, src/test/kotlin/io/github/brdoliveira/creditflow/domain, src/test/kotlin/io/github/brdoliveira/creditflow/application, src/test/kotlin/io/github/brdoliveira/creditflow/evaluation, docs/architecture.md
  mensagem de commit: "T-037 fortalecer-arquitetura-modular: Consolidar a organização por feature"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high &&
    rodar_tarefa 'faixa-1' 'T-040' 'Você executa UMA tarefa da feature "fortalecer-arquitetura-modular" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fortalecer-arquitetura-modular/spec.md, .spec/features/fortalecer-arquitetura-modular/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-040 — "Atualizar documentação e provar compatibilidade"
  critérios/refs: AC-095 (Documentação e contratos continuam coerentes)
  arquivos permitidos (e seus testes): docs/architecture.md, docs/adrs/001-modular-monolith.md, test/kotlin-architecture.test.mjs, src/test/kotlin, test
  mensagem de commit: "T-040 fortalecer-arquitetura-modular: Atualizar documentação e provar compatibilidade"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/fortalecer-arquitetura-modular-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-036 T-037 T-040
  return 0
}
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium &&
    rodar_tarefa 'faixa-2' 'T-039' 'Você executa UMA tarefa da feature "fortalecer-arquitetura-modular" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/fortalecer-arquitetura-modular/spec.md, .spec/features/fortalecer-arquitetura-modular/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-039 — "Tornar a classe Kotlin a fonte do evento da outbox"
  critérios/refs: AC-094 (Evento da outbox possui uma única fonte de contrato)
  arquivos permitidos (e seus testes): src/main/kotlin/io/github/brdoliveira/creditflow/evaluation/application/event/CreditEvaluationCompleted.kt, src/main/kotlin/io/github/brdoliveira/creditflow/evaluation/infrastructure/persistence/PostgresCreditEvaluationRepository.kt, src/main/resources/db/migration/V5__explicit_credit_outbox.sql, src/test/kotlin/io/github/brdoliveira/creditflow/infrastructure/messaging/CreditEvaluationMessagingIT.kt, src/test/kotlin/io/github/brdoliveira/creditflow/infrastructure/messaging/OutboxKafkaIT.kt
  mensagem de commit: "T-039 fortalecer-arquitetura-modular: Tornar a classe Kotlin a fonte do evento da outbox"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/fortalecer-arquitetura-modular-faixa-2' "$WT" "$st" || return 1
  marcar_concluidas T-038 T-039
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/fortalecer-arquitetura-modular/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-036, T-037, T-040"
  echo "  faixa-2  onda 1  T-038, T-039"
  echo
