...
import io.github.brdoliveira.creditflow.evaluation.infrastructure.web.dto.RuleResponse
...
class CreditEvaluationWebMapper {
    ...
    fun toResponse(value: CreditEvaluation, customerName: String = "Cliente") = CreditEvaluationResponse(
        value.evaluationId,
        customerName,
        value.maskedCpf,
        value.decision.name,
        ...
        value.ruleResults.map { RuleResponse(it.code, it.name, it.status.name, it.reason) },
        value.processedAt.atOffset(ZoneOffset.UTC),
