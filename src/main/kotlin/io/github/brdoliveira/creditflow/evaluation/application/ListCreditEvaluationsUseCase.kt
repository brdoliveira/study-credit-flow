...
class ListCreditEvaluationsUseCase(private val repository: CreditEvaluationRepository) {
    ...
    fun execute(
        filter: CreditEvaluationFilter = CreditEvaluationFilter(),
        page: CreditEvaluationPageRequest = CreditEvaluationPageRequest(),
    ): CreditEvaluationPage = repository.findPage(filter, page)

    /** Lista avaliações a partir dos parâmetros HTTP já normalizados. */
    fun execute(decision: String?, from: Instant?, to: Instant?, page: Int, size: Int): CreditEvaluationPage =
        execute(CreditEvaluationFilter(decision, from, to), CreditEvaluationPageRequest(page, size))
}
